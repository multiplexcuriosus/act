...
The learned action is trajectory_executor's one-dimensional captured-line
coordinate ``s``:

    action(t) = 0.0                 before the first executed CMD_GOTO_S
    action(t) = latest commanded s  at/after an executed CMD_GOTO_S
...
def sample_episode(
    ...
    episode: EpisodeWindow,
    fps: float,
    strict_single_command: bool,
) -> Dict[str, np.ndarray]:
    """Causally sample observations and densify sparse GOTO_S commands."""
    command_count = len(data["goto_s_t"])
    if strict_single_command and command_count != 1:
        raise RuntimeError(
            f"Episode {episode.output_idx}: expected exactly one GOTO_S command, "
            f"found {command_count}. Use --allow_multiple_commands only if intentional."
        )
    if command_count > 1:
        log(
            f"[WARNING] episode {episode.output_idx}: {command_count} GOTO_S "
            "commands will be represented by causal zero-order hold"
        )
    ...
    command_indices = np.searchsorted(command_times, grid, side="right") - 1
    ...
    action = np.zeros((grid.size, 1), dtype=np.float32)
    action_is_commanded = np.zeros((grid.size, 1), dtype=np.uint8)
    commanded = command_indices >= 0
    action[commanded, 0] = command_values[command_indices[commanded]]
    action_is_commanded[commanded, 0] = 1

    first_grid_command_index = int(np.flatnonzero(commanded)[0]) if np.any(commanded) else -1
    if first_grid_command_index < 0:
        raise RuntimeError(
            f"Episode {episode.output_idx}: GOTO_S exists inside episode but occurs "
            "after the causally usable RGB/joint interval"
        )

    log(
        f"[INFO] episode {episode.output_idx}: sampled {grid.size} steps at "
        f"{fps:.3f} Hz; first command at sample {first_grid_command_index}, "
        f"s={action[first_grid_command_index, 0]:+.6f} m"
    )
    ...
    return {
        ...
        "qpos": qpos,
        "action": action,
        "action_is_commanded": action_is_commanded,
...
def write_episode(
    ...
    try:
        with h5py.File(temporary_path, "w") as h5:
            ...
            h5.create_dataset(
                "action_is_commanded",
                data=arrays["action_is_commanded"],
                dtype=np.uint8,
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Allow multiple GOTO_S commands in one episode and zero-order-hold each",
    )
    parser.add_argument(
        "--no_target_base",
        action="store_true",
...
def main() -> None:
    ...
    for episode in episodes:
        ...
        arrays = sample_episode(
            ...
            episode=episode,
            fps=args.fps,
            strict_single_command=not args.allow_multiple_commands,
        )
