...
SPARSE_BALL_FEATURE_NAMES = SPARSE_FEATURE_NAMES
...
def validate_policy_rate(policy_rate_hz) -> int:
    """Return a supported integral rollout policy rate."""
...
def rate_contract(policy_rate_hz):
    """Return causal frame offsets, one-second chunk size, and timer period."""
    rate = validate_policy_rate(policy_rate_hz)
    offsets = tuple(int(round(value * rate)) for value in SPARSE_HISTORY_OFFSETS_SEC)
    return offsets, rate, 1.0 / rate
...
BallObservation = SparsePoint


def sparse_feature_at_time(points, target_time, image_width, image_height,
                           max_observation_age_sec=DEFAULT_MAX_OBSERVATION_AGE_SEC):
    """Construct one causal feature row using the committed normalization."""
    return construct_causal_sparse_history(
        points, target_time, (0.0,), image_width, image_height,
...
def validate_sparse_checkpoint_contract(
    ...
    if policy_rate_hz is not None:
        rate = validate_policy_rate(policy_rate_hz)
        saved_rate = stats.get("policy_rate_hz")
        if saved_rate is None or validate_policy_rate(saved_rate) != rate:
            raise ValueError(
        ...
        if saved_offsets is None or tuple(saved_offsets) != expected_offsets:
            raise ValueError(
