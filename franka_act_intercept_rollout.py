...
try:
    # Generated by /openmv_cam/msg/EventTrackerUpdate.msg.
    from openmv_cam.msg import EventTrackerUpdate
except ImportError:  # Allows explicitly requested legacy PointStamped deployments.
    EventTrackerUpdate = None
...
class BufferedEventObservation:
    """One received tracker result; invalid results are retained for provenance."""

    tracker_update_id: Optional[int]
    tracker_update_id_valid: bool
    source_packet_id: Optional[int]
    source_packet_id_valid: bool
    source_timestamp_ns: int
    receipt_timestamp_ns: int
    sensor_window_start_us: Optional[int]
    sensor_window_end_us: Optional[int]
    x_px: float
    y_px: float
    ...
    window_event_count: Optional[int]
    confidence: Optional[float]
    velocity_valid: bool
    legacy_pointstamped: bool = False

    @property
    def source_timestamp(self) -> float:
        return self.source_timestamp_ns * 1e-9
...
class FrankaActRolloutNode(Node):
    def __init__(self, args):
        ...
        self.legacy_event_pointstamped = bool(args.legacy_event_pointstamped)
        ...
        self.event_update_buffer: Deque[BufferedEventObservation] = deque(
            maxlen=self.max_source_buffer
        )
        ...
        # Compatibility-only local sequence; never a hardware packet ID.
        # It advances only when the selected source timestamp changes.
        ...
        self._last_latest_event_signature = None
        ...
        if self.input_modality == "sparse_ball":
            if self.sparse_source == "event" and not self.legacy_event_pointstamped:
                if EventTrackerUpdate is None:
                    raise RuntimeError(
                        "openmv_cam.msg.EventTrackerUpdate is unavailable; source the built "
                        "openmv_cam workspace or pass --legacy_event_pointstamped"
                    )
                self.create_subscription(
                    EventTrackerUpdate, self.event_update_topic,
                    self.event_tracker_update_cb, 10,
                )
            else:
        ...
        if self.input_modality == "sparse_ball":
            ...
            if self.sparse_source == "event":
                self.get_logger().info(
                    "event input subscription="
                    + (f"legacy PointStamped {self.sparse_topic}"
                       if self.legacy_event_pointstamped
                       else f"openmv_cam/msg/EventTrackerUpdate {self.event_update_topic}")
                )
        else:
    ...
    def sparse_cb(self, msg: PointStamped) -> None:
        """Legacy fallback: PointStamped cannot express invalid updates or packet IDs."""
        t = stamp_to_sec(msg.header.stamp)
        ...
        if self.sparse_source == "event":
            ...
            self.event_update_buffer.append(BufferedEventObservation(
                tracker_update_id=None,
                tracker_update_id_valid=False,
                source_packet_id=None,
                source_packet_id_valid=False,
                source_timestamp_ns=stamp_to_ns(msg.header.stamp),
                receipt_timestamp_ns=receipt_ns,
                sensor_window_start_us=None,
                sensor_window_end_us=None,
                x_px=float(msg.point.x),
                y_px=float(msg.point.y),
                vx_px_s=None,
                vy_px_s=None,
                valid=True,
                rejection_reason="legacy_pointstamped_no_tracker_validity",
                candidate_count=None,
                window_event_count=None,
                confidence=None,
                velocity_valid=False,
                legacy_pointstamped=True,
            ))

    def event_tracker_update_cb(self, msg) -> None:
        """Retain every typed update, while preserving held-valid policy semantics."""
        receipt_ns = int(self.get_clock().now().nanoseconds)
        header_ns = stamp_to_ns(msg.header.stamp)
        availability_ns = int(msg.availability_timestamp_ns)
        source_ns = availability_ns if availability_ns > 0 else header_ns
        observation = BufferedEventObservation(
            tracker_update_id=int(msg.tracker_update_id),
            tracker_update_id_valid=bool(msg.tracker_update_id_valid),
            source_packet_id=(int(msg.source_packet_id)
                              if bool(msg.source_packet_id_valid) else None),
            source_packet_id_valid=bool(msg.source_packet_id_valid),
            source_timestamp_ns=source_ns,
            receipt_timestamp_ns=receipt_ns,
            sensor_window_start_us=int(msg.sensor_window_start_us),
            sensor_window_end_us=int(msg.sensor_window_end_us),
            x_px=float(msg.x_px), y_px=float(msg.y_px),
            vx_px_s=float(msg.vx_px_s), vy_px_s=float(msg.vy_px_s),
            valid=bool(msg.valid), rejection_reason=str(msg.rejection_reason),
            candidate_count=int(msg.candidate_count),
            window_event_count=int(msg.window_event_count),
            confidence=float(msg.confidence), velocity_valid=bool(msg.velocity_valid),
        )
    ...
    def _build_sparse_policy_inputs(self, policy_time: float):
        ...
        def observation_from_visual(ts, msg):
            if isinstance(msg, BufferedEventObservation):
                return msg
            return BufferedEventObservation(
                tracker_update_id=None, tracker_update_id_valid=False,
                source_packet_id=None,
                source_packet_id_valid=False,
                source_timestamp_ns=int(round(ts * 1e9)), receipt_timestamp_ns=0,
                sensor_window_start_us=None, sensor_window_end_us=None,
                x_px=float(msg.point.x), y_px=float(msg.point.y),
                vx_px_s=None, vy_px_s=None, valid=True,
                rejection_reason="legacy_pointstamped_no_tracker_validity",
                candidate_count=None, window_event_count=None, confidence=None,
                velocity_valid=False, legacy_pointstamped=True,
            )

        observations = sorted(
            (observation_from_visual(ts, msg) for ts, msg in self.visual_buffer),
            key=lambda obs: obs.source_timestamp_ns,
        )
        sparse_points = [SparsePoint(
            obs.source_timestamp, obs.x_px, obs.y_px, int(obs.valid)
        ...
        latest_update = (
            update_buffer[-1]
            if self.sparse_source == "event" and update_buffer else None
        )
        policy_timestamp_ns = int(round(policy_time * 1e9))
        # Selection is evaluated against the already captured policy ROS time.
        selection_timestamp_ns = policy_timestamp_ns
        self._trace_mark(
            "event_tracker_update_available",
            tracker_latest_update_id=(None if latest_update is None
                                      else latest_update.tracker_update_id),
        )
        source_timestamp_ns = (
        ...
        rollout_geometry_age_valid = bool(
        ...
        tracker_valid = bool(selected_observation is not None and selected_observation.valid)
        policy_selected_valid = bool(tracker_valid and rollout_geometry_age_valid)
        selected_signature = (
            None if selected_observation is None else
            (selected_observation.tracker_update_id,
             selected_observation.source_timestamp_ns,
             selected_observation.x_px, selected_observation.y_px,
             selected_observation.valid)
        )
        previous_selected_signature = getattr(self, "_last_selected_event_signature", None)
        event_policy_observation_changed = selected_signature != previous_selected_signature
        event_source_changed = source_timestamp_ns != getattr(
            self, "_last_selected_event_source_timestamp_ns", None,
        )
        if source_timestamp_ns is not None and event_source_changed:
            self._event_observation_sequence = getattr(
                self, "_event_observation_sequence", 0) + 1
        self._last_selected_event_source_timestamp_ns = source_timestamp_ns
        self._last_selected_event_signature = selected_signature
        latest_signature = (
            None if latest_update is None else
            (latest_update.tracker_update_id, latest_update.source_timestamp_ns)
        )
        latest_tracker_update_changed = latest_signature != getattr(
            self, "_last_latest_event_signature", None,
        )
        self._last_latest_event_signature = latest_signature
        selected_is_held = bool(
            selected_observation is not None and event_age_sec is not None
            and event_age_sec > 0.0
        )
        selected_hold_duration_sec = event_age_sec if selected_is_held else 0.0
        self._trace_mark(
            "event_observation_selected",
            policy_tick_timestamp_ns=policy_timestamp_ns,
            observation_selection_timestamp_ns=selection_timestamp_ns,
        ...
        latest_age_sec = (
            None if latest_update is None else
            float(policy_time - latest_update.source_timestamp)
        )
        provenance = {
            "policy_tick_timestamp_ns": policy_timestamp_ns,
            "observation_selection_timestamp_ns": selection_timestamp_ns,
            "tracker_latest_update_id": obs_value(latest_update, "tracker_update_id"),
            "tracker_latest_source_packet_id": obs_value(latest_update, "source_packet_id"),
            "tracker_latest_source_packet_id_valid": bool(
                latest_update is not None and latest_update.source_packet_id_valid),
            "tracker_latest_timestamp_ns": obs_value(latest_update, "source_timestamp_ns"),
            "tracker_latest_receipt_timestamp_ns": obs_value(latest_update, "receipt_timestamp_ns"),
            "tracker_latest_age_sec": latest_age_sec,
            "tracker_latest_u": obs_value(latest_update, "x_px"),
            "tracker_latest_v": obs_value(latest_update, "y_px"),
            "tracker_latest_valid": bool(latest_update is not None and latest_update.valid),
            "tracker_latest_rejection_reason": obs_value(latest_update, "rejection_reason"),
            "tracker_latest_window_event_count": obs_value(latest_update, "window_event_count"),
            "policy_selected_update_id": obs_value(selected_observation, "tracker_update_id"),
            "policy_selected_source_packet_id": obs_value(selected_observation, "source_packet_id"),
            "policy_selected_source_packet_id_valid": bool(
                selected_observation is not None and selected_observation.source_packet_id_valid),
            "policy_selected_timestamp_ns": obs_value(selected_observation, "source_timestamp_ns"),
            "policy_selected_receipt_timestamp_ns": obs_value(selected_observation, "receipt_timestamp_ns"),
            "policy_selected_age_sec": event_age_sec,
            "policy_selected_u": event_u,
            "policy_selected_v": event_v,
            "tracker_valid": tracker_valid,
            "rollout_geometry_age_valid": rollout_geometry_age_valid,
            "policy_selected_valid": policy_selected_valid,
            "policy_selected_is_held": selected_is_held,
            "policy_selected_hold_duration_sec": selected_hold_duration_sec,
            "policy_selected_rejection_reason": obs_value(selected_observation, "rejection_reason"),
            "selected_event_source_timestamps_ns": [obs_value(obs, "source_timestamp_ns")
                                                    for obs in selected_observations],
            "selected_event_update_ids": [obs_value(obs, "tracker_update_id")
                                          for obs in selected_observations],
            "selected_event_ages_sec": selected_event_ages,
            ...
            "latest_tracker_update_changed": latest_tracker_update_changed,
            # Compatibility aliases for traces produced by the earlier rollout.
            "event_source_timestamp_ns": source_timestamp_ns,
            "event_age_sec": event_age_sec,
            "event_u": event_u, "event_v": event_v,
            "event_valid": policy_selected_valid,
            "event_input_changed": event_policy_observation_changed,
            # Local rollout sequence (legacy correlation only), never a hardware packet ID.
    ...
    def run_policy_step(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            self._trace_add_detail(
                ...
                prediction_chunk_size=int(self.chunk_size),
                temporal_aggregation_mode=str(self.temporal_agg_mode),
                temporal_aggregation_contributor_count=int(selection.contributor_count),
                temporal_aggregation_effective_age_frames=float(selection.effective_age_frames),
            )
...
def main():
    ...
    parser.add_argument(
        "--event_update_topic", type=str,
        default="/openmv_cam/event_tracker/update",
        help="Typed openmv_cam/msg/EventTrackerUpdate topic for sparse event rollout.",
    )
    parser.add_argument(
        "--legacy_event_pointstamped", action="store_true",
        help="Use --sparse_topic PointStamped fallback; tracker validity/IDs are unavailable.",
    )
