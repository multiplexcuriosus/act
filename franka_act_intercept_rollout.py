...
class FrankaActRolloutNode(Node):
    ...
    def build_policy_inputs(self):
        ...
        self._trace_mark(
            ...
            selected_history_timestamps_ns=[int(round(ts * 1e9)) for ts in sync.visual_timestamps],
    ...
    def timer_cb(self) -> None:
        source_stamp_ns = 0
        if self.visual_buffer:
            source_stamp_ns = image_source_stamp_ns(self.visual_buffer[-1][1])
        trace = self.latency_tracer.begin(source_stamp_ns)
        self._active_latency_trace = trace
        valid = False
        rejection_reason = ""
        if not self.ready():
            rejection_reason = "waiting for visual image, JointState, and current_tcp_s streams"
