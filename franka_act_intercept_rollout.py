...
PRODUCTION_NODE_NAME = "franka_act_rollout_intercept"
PRODUCTION_PREDICTION_TOPIC = "/act/intercept_prediction_chunk_abs_s"
PRODUCTION_CURRENT_PREDICTION_TOPIC = "/act/intercept_prediction_current_abs_s"
...
def resolve_rollout_runtime(sparse_source: Optional[str], dryrun: Optional[str]) -> Dict[str, Any]:
    ...
    if resolved["dryrun"]:
        ...
        assert resolved["prediction_current_topic"] != PRODUCTION_CURRENT_PREDICTION_TOPIC
    return resolved
...
class FrankaActRolloutNode(Node):
    def __init__(self, args):
        if args.node_namespace:
            super().__init__(PRODUCTION_NODE_NAME, namespace=args.node_namespace)
        else:
            super().__init__(PRODUCTION_NODE_NAME)
